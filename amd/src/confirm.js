...
define([
  ...
  var reloadList = function(data, list) {
    templates.render('tool_mitxel/entries_list', data).done(function(html) {
  ...
  var processDelete = function(id, list) {
    ...
    var requests = ajax.call([{
      methodname: 'tool_mitxel_delete_entry',
      args: {id: id}
    }, {
      methodname: 'tool_mitxel_entries_list',
  ...
  var confirmDelete = function(id, list) {
    str.get_strings([
      {key: 'delete'},
      {key: 'confirmdeleteentry', component: 'tool_mitxel'},
      {key: 'yes'},
      {key: 'no'}
    ]).done(function(s) {
  ...
  var registerClickHandler = function(selector) {
    $(selector).on('click', function(e) {
      ...
      var id = $(e.currentTarget).attr('data-entryid'),
        list = $(e.currentTarget).closest('.tool_mitxel_entries_list');
      confirmDelete(id, list);
    });
  };

  return /** @alias module:tool_mitxel/confirm */ {
    /**
